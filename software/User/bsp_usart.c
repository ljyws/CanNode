...
//uint8_t uart4_buf[UART4_BUFLEN];
//uint8_t uart5_buf[UART5_BUFLEN];
...
//    {
//      /* 进入空闲中断回调函数 */
//      usart4_callback_handler(usart6_buf);
//    }
...
//  {
//    //判断数据是否为期望的长度 如不是则不进入回调函数 直接开启下一次接收
//    if ((UART4_MAX_LEN - dma_current_data_counter(huart->hdmarx->Instance)) == UART4_BUFLEN)
//    {
//      /* 进入空闲中断回调函数 */
//      usart4_callback_handler(uart4_buf);
//    }

//    __HAL_DMA_SET_COUNTER(huart->hdmarx, UART4_MAX_LEN);
//  }
//  else if (huart == &huart5)
//  {
//    //判断数据是否为期望的长度 如不是则不进入回调函数 直接开启下一次接收
//    if ((UART5_MAX_LEN - dma_current_data_counter(huart->hdmarx->Instance)) == UART5_BUFLEN)
//    {
//      /* 进入空闲中断回调函数 */
//      usart5_callback_handler(uart5_buf);
//    }

//    /* 设置DMA接收数据的长度 */
//    __HAL_DMA_SET_COUNTER(huart->hdmarx, UART5_MAX_LEN);
//  }
}
...
//    uart_receive_dma_no_it(&huart4, uart5_buf, UART4_MAX_LEN);
//  }
//  else if (huart == &huart5)
//  {
//    __HAL_UART_CLEAR_IDLEFLAG(&huart5);
//    __HAL_UART_ENABLE_IT(&huart6, UART_IT_IDLE);
//    uart_receive_dma_no_it(&huart5, uart5_buf, UART5_MAX_LEN);
//  }
}
...
/**
  ...
  * @retval	none
  */
//用哪个开哪个，不需要一次次往main初始化里搬
void uart_init(void)
{
//	uart_receive_init(&huart1);
//	uart_receive_init(&huart2);
	uart_receive_init(&huart3);
//	uart_receive_init(&huart4);
//	uart_receive_init(&huart5);
//	uart_receive_init(&huart6);	
}
